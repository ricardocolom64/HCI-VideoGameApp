...
if add_selectbox == "Homepage":
    ...
    if ('Genre' in selectTableInfo):
        ...
        selectGenre = st.selectbox(
            ...
            ('All', 'Action', 'Adventure', 'Arcade', 'Board Games', 'Card', 'Casual', 'Educational', 'Family',
             'Fighting',
             'Indie', 'Massively Multiplayer', 'Platformer', 'Puzzle', 'Racing', 'RPG', 'Simulation', 'Shooter',
             'Sports', 'Strategy'),
            on_change=fixPage, args=("Reset",)
        )
    ...
    def ratings_data(game_to_search_for):
        ...
        if found_game:
            ...
            if reviewers > 0:
                ...
                game_return = [True, found_game["name"], ratings_list, found_game["background_image"],
                               found_game["rating"], found_game["genres"]]
            else:
                game_return = [True, found_game["name"], [0], found_game["background_image"], found_game["rating"],
                               found_game["genres"]]
    ...
    def compare_game(game1, game2):
        ...
        if game1_ratings[0] and game2_ratings[0]:
            ...
            if not game1_ratings[0] and not game2_ratings[0]:
                ...
                st.error("Game " + str(
                    game1) + " not found. Please try spelling it a different way, or try a different game.")
            elif not game2_ratings[0]:
                st.error("Game " + str(
                    game2) + " not found. Please try spelling it a different way, or try a different game.")
...
# ------------- LOCATIONS PAGE -------------
    ...
    # Search by user's actual location
        ...
        bestBuyLocationURL = "https://api.bestbuy.com/v1/stores((area(" + str(userLat) + "," + str(
            userLong) + ",10)))?apiKey=" + keys.BESTBUY_API_KEY + "&show=lng,lat,name&format=json"
        bestBuyLocations = requests.get(bestBuyLocationURL).json()
